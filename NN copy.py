...
class NeuralNetwork():
	...
	def objective_function__definition_BO(self,x,y):
		...
		def objective_function(params_dict):
			estimator = KerasClassifier(self.new_model, **params_dict)
    ...
    def hp_tuning_BO(self, x, y, iterations=10, **kwargs):
		...
		#For compatibility with HyperOpt is best to pass them using HyperOpt distribution
		objective_function = self.objective_function__definition_BO(x,y)
		trials = Trials()
		param_space = kwargs
		...
		print("Keys for trials.trials are: ")
		print(trials.trials.keys())
    	loss = [x['result']['loss'] for x in trials.trials]
		best_param_values = [x for x in best_param.values()]
		print("Best score obtained: %f\n with parameters: %s", %(min(loss)*-1, best_param_values))
        return trials
